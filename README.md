...
# Презентация
В LaTex можно использовать шаблоны для написания презентаций. Также можно самим написать презентацию, например:
```
...
\end{document}
```
Сначала пишем тип документа `\documentclass{beamer}`, **beamer** - класс для ЛаТеха, предназначенный для создания слайдов и презентаций, затем идет преамбула, `\title{Заголовок}`, `\author{name}`, начало и конец слайда пишем через команды `\begin{frame}`, `\end{frame}`. Команда `\titlepage` по умолчанию создает слайд, на котором отображаются:
+ название
+ дата 
+ автор
+ учреждение
+ если есть, то логотип


**Темы оформления** — это набор файлов .sty, размещенных там, где их может найти компилятор, т.е. там же, где и сам файл презентации. Подключается тема соответствующей строкой в преамбуле презентации. Например: `\usetheme{gcr2019}`, здесь в скобках название темы презентации.

В презентации можно вставлять изображения. 
```
\begin{figure}
...
И логотип: 
```
\logo{\includegraphics[height=1cm]{логотип}}
```
Как и в документах в презетации можно сделать слайд с содержанием:
```
\section*{Содержание}
...
\end{frame}
```
Для набора текста на слайдах в колонки служит окружение `columns`:
```
\begin{columns}
\column{.aa\textwidth}
First column text
...
\end{columns}
```
Блоки используют для выделения отдельных фрагментов текста:
```
...
\end{block}  
```
При желании можно объединять блоки и колонки.
Для написания списка литературы используем пакет biblatex, но beamer перекрывает часть умолчаний biblatex.
